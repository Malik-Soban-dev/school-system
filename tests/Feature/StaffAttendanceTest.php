...
class StaffAttendanceTest extends TestCase
{
    ...
    public function test_staff_attendance_is_unique_per_day_and_teacher_sees_own_history(): void
    {
        ...
        $id = $this->actingAs($owner)->postJson('/portal/records/staff_attendance', ['user_id' => $teacher->id, 'date' => $date, 'status' => 'late', 'check_in' => '08:35', 'note' => 'Traffic'])->assertOk()->json('id');
        ...
        $this->actingAs($owner)->postJson('/portal/records/staff_attendance', ['user_id' => $teacher->id, 'date' => $date, 'status' => 'present'])->assertUnprocessable()->assertJsonValidationErrors('user_id');
