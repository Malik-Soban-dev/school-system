...
class SuperadminAccessTest extends TestCase
{
    ...
    public function test_superadmin_can_open_platform_dashboard(): void
    {
        ...
        $this->actingAs($user)->get('/superadmin')->assertOk()->assertSee('Superadmin dashboard')->assertSee('Account security')->assertSee('Search school or slug')->assertSee('Invitation roles');
    }
    ...
    public function test_superadmin_can_review_a_paginated_cross_school_branch_registry(): void
    {
        ...
        $this->actingAs($superadmin)->getJson('/superadmin/branches?school_id='.$school.'&status=active&search=North&per_page=1')->assertOk()->assertJsonPath('branches.total', 1)->assertJsonPath('branches.data.0.school_name', 'Branch Registry School')->assertJsonPath('branches.data.0.name', 'North Branch')->assertJsonPath('branches.data.0.students', 0)->assertJsonPath('branches.data.0.guardians', 0)->assertJsonPath('branches.data.0.classes', 0)->assertJsonPath('branches.data.0.subjects', 0)->assertJsonPath('branches.data.0.attendance', 0)->assertJsonPath('branches.data.0.exams', 0)->assertJsonPath('branches.data.0.payments', 0)->assertJsonPath('branches.data.0.notifications', 0)->assertJsonPath('branches.data.0.open_invoices', 0)->assertJsonStructure(['branches' => ['data' => [['academic_years', 'subjects', 'attendance', 'timetables', 'exams', 'exam_subjects', 'grade_bands', 'grades', 'enrollments', 'payments', 'expenses', 'leave_requests', 'payroll', 'payroll_payments', 'notices', 'invitations', 'notifications', 'notification_events', 'notification_deliveries', 'notification_preferences']]]]);
    }
    ...
    public function test_superadmin_platform_summary_exposes_the_full_operational_census(): void
    {
        ...
        $this->actingAs($superadmin)->getJson('/superadmin/data')->assertOk()->assertJsonStructure([
            'summary' => [
                ...
                'notification_events', 'notification_deliveries', 'notification_preferences', 'settings', 'audit', 'entitlement_alerts',
            ],
        ]);
    }
    ...
    public function test_superadmin_school_detail_exposes_complete_operational_census(): void
    {
        ...
        $this->actingAs($superadmin)->getJson('/superadmin/schools/'.$school)->assertOk()->assertJsonPath('counts.members', 0)->assertJsonPath('counts.students', 0)->assertJsonPath('counts.teachers', 0)->assertJsonPath('counts.guardians', 0)->assertJsonPath('counts.enrollments', 0)->assertJsonPath('counts.payroll_payments', 0)->assertJsonPath('counts.notifications', 0)->assertJsonStructure(['counts' => ['members', 'students', 'staff', 'teachers', 'guardians', 'classes', 'academic_years', 'subjects', 'guardian_links', 'teacher_assignments', 'attendance', 'timetables', 'exams', 'exam_subjects', 'grade_bands', 'grades', 'enrollments', 'invoices', 'open_invoices', 'payments', 'expenses', 'leave_requests', 'payroll', 'payroll_payments', 'notices', 'invitations', 'notifications', 'notification_events', 'notification_deliveries', 'notification_preferences', 'settings', 'audit']]);
    }
