...
(() => {
    ...
    const userSchoolStatus = document.querySelector('#user-school-status');
    const userBranchStatus = document.querySelector('#user-branch-status');
    ...
    const loadHealth = async () => {
        ...
        const storageLabel = data.storage?.available ? `${esc(data.storage.files)} files / ${esc(data.storage.bytes)} bytes` : 'unavailable';
        const scheduledRows = (data.scheduled_runs || []).map(run => `<tr><td>${esc(run.command)}</td><td><span class="platform-status ${esc(run.status)}">${esc(run.status)}</span></td><td>${esc(run.started_at)}</td><td>${esc(run.finished_at || 'running')}</td><td>${esc(run.error_type || '—')}</td></tr>`).join('') || '<tr><td colspan="5">No recorded scheduler runs yet.</td></tr>';
        const errorRows = (data.application_errors?.recent || []).map(error => `<tr><td>${esc(error.error_type)}</td><td>${esc(error.status)}</td><td>${esc(error.method || '—')}</td><td>${esc(error.route || 'unrouted')}</td><td>${esc(error.occurred_at)}</td></tr>`).join('') || '<tr><td colspan="5">No recorded application errors.</td></tr>';
        const notificationStatus = data.notification_deliveries?.available ? (data.notification_deliveries.by_status || []).map(item => `${esc(item.status)}: <strong>${esc(item.total)}</strong>`).join(' · ') || 'none recorded' : 'unavailable';
        const notificationFailureRows = (data.notification_deliveries?.recent_failures || []).map(item => `<tr><td>${esc(item.school_name)}</td><td>${esc(item.branch_name || '—')}</td><td>${esc(item.channel)}</td><td><span class="platform-status ${esc(item.status)}">${esc(item.status)}</span></td><td>${esc(item.attempts)}</td><td>${esc(item.error_code || '—')}</td><td>${esc(item.updated_at)}</td></tr>`).join('') || '<tr><td colspan="7">No recent notification delivery failures.</td></tr>';
        healthContent.innerHTML = `<p><span class="platform-status ${esc(data.status)}">${esc(data.status)}</span> Database: <strong>${esc(data.database)}</strong> · Active schools: <strong>${esc(data.schools.active)}</strong> · Suspended schools: <strong>${esc(data.schools.suspended)}</strong> · Pending jobs: <strong>${esc(data.queue.pending)}</strong> · Failed jobs: <strong>${esc(data.queue.failed)}</strong> · Sessions: <strong>${esc(data.sessions ?? 'not configured')}</strong> · Private storage: <strong>${storageLabel}</strong> · Errors (24h): <strong>${esc(data.application_errors?.last_24h ?? 'unavailable')}</strong> · Notification activity (24h): <strong>${esc(data.notification_deliveries?.last_24h ?? 'unavailable')}</strong> · Last audit: <strong>${esc(data.last_audit_at || 'none')}</strong></p><h3>Notification delivery status</h3><p>${notificationStatus}</p><h3>Recent notification failures</h3><div class="platform-table-wrap"><table><thead><tr><th>School</th><th>Branch</th><th>Channel</th><th>Status</th><th>Attempts</th><th>Error code</th><th>Updated</th></tr></thead><tbody>${notificationFailureRows}</tbody></table></div><h3>Recent scheduled runs</h3><div class="platform-table-wrap"><table><thead><tr><th>Command</th><th>Status</th><th>Started</th><th>Finished</th><th>Error type</th></tr></thead><tbody>${scheduledRows}</tbody></table></div><h3>Recent application errors</h3><div class="platform-table-wrap"><table><thead><tr><th>Type</th><th>Status</th><th>Method</th><th>Route</th><th>Occurred</th></tr></thead><tbody>${errorRows}</tbody></table></div><h3>Recent encrypted backups</h3><div class="platform-table-wrap"><table><thead><tr><th>File</th><th>Size</th><th>Modified</th><th>Control</th></tr></thead><tbody>${backupRows}</tbody></table></div>`;
    ...
    const showSchoolDetail = async schoolId => {
        ...
        const featureLabels = {attendance: 'Attendance', grades: 'Grades and exams', invoices: 'Invoices and payments', payroll: 'Payroll', notifications: 'Notifications', branches: 'Additional branches'};
        const featureRows = Object.entries(featureLabels).map(([feature, label]) => { const override = data.feature_overrides?.[feature]; const selected = override === undefined ? '' : (Number(override) === 1 ? 'enabled' : 'disabled'); return `<tr><td>${esc(label)}<small>${esc(feature)}</small></td><td><strong>${data.feature_access?.[feature] ? 'Enabled' : 'Disabled'}</strong></td><td><form class="feature-override-form" data-feature="${esc(feature)}"><select name="enabled" aria-label="${esc(label)} access"><option value="" ${selected === '' ? 'selected' : ''}>Inherit plan</option><option value="true" ${selected === 'enabled' ? 'selected' : ''}>Enabled</option><option value="false" ${selected === 'disabled' ? 'selected' : ''}>Disabled</option></select><button type="submit">Save</button></form></td></tr>`; }).join('');
        const billingRows = (data.billing?.invoices || []).map(invoice => `<tr><td>${esc(invoice.invoice_number)}</td><td>${esc(invoice.status)}</td><td>${esc(invoice.currency)} ${(Number(invoice.amount_cents) / 100).toFixed(2)}</td><td>${esc(invoice.due_on)}</td><td>${esc(invoice.paid_at || '—')}</td><td>${esc(invoice.payment_reference || '—')}</td></tr>`).join('') || '<tr><td colspan="6">No platform invoices recorded.</td></tr>';
        const branchOptions = data.branches.map(branch => `<option value="${esc(branch.id)}">${esc(branch.name)} (${esc(branch.code)})</option>`).join('');
        const roleOptions = [['owner', 'Owner'], ['admin', 'Admin'], ['teacher', 'Teacher'], ['student', 'Student'], ['parent', 'Parent'], ['accountant', 'Accountant']].map(([value, label]) => `<option value="${value}">${label}</option>`).join('');
         const branchRows = data.branches.map(branch => `<tr><td>${esc(branch.name)}${branch.is_default ? ' <small>Default</small>' : ''}</td><td>${esc(branch.code)}</td><td><span class="platform-status ${esc(branch.status)}">${esc(branch.status)}</span></td><td>${esc(branch.members)} members · ${esc(branch.students)} students · ${esc(branch.staff)} staff · ${esc(branch.teachers)} teachers · ${esc(branch.classes)} classes · ${esc(branch.subjects)} subjects · ${esc(branch.attendance)} attendance · ${esc(branch.exams)} exams · ${esc(branch.grades)} grades · ${esc(branch.payments)} payments · ${esc(branch.payroll)} payroll · ${esc(branch.notifications)} notifications · ${esc(branch.open_invoices)} open invoices</td><td><button class="branch-workspace" data-branch="${esc(branch.id)}">Open workspace</button> <button class="branch-edit" data-branch="${esc(branch.id)}" data-name="${esc(branch.name)}" data-code="${esc(branch.code)}">Edit</button> ${branch.is_default ? '<small>Default</small>' : `<button class="branch-default" data-branch="${esc(branch.id)}">Set default</button>`} <button class="branch-status" data-branch="${esc(branch.id)}" data-status="${branch.status === 'active' ? 'suspended' : 'active'}">${branch.status === 'active' ? 'Suspend' : 'Activate'}</button></td></tr>`).join('') || '<tr><td colspan="5">No branches registered.</td></tr>';
        const members = data.members.map(member => `<tr><td>${esc(member.name)}</td><td>${esc(member.email)}</td><td>${esc(member.is_active ? 'Account active' : 'Account inactive')} · ${esc(member.membership_status)}</td><td><button class="membership-status" data-user="${esc(member.id)}" data-status="${member.membership_status === 'active' ? 'suspended' : 'active'}">${member.membership_status === 'active' ? 'Suspend school access' : 'Activate school access'}</button><form class="branch-access-form" data-school="${esc(data.school.id)}" data-user="${esc(member.id)}"><select name="branch_id" aria-label="Branch for ${esc(member.name)}">${branchOptions}</select><select name="roles" aria-label="Roles for ${esc(member.name)}" required multiple size="3">${roleOptions}</select><button type="submit">Grant access</button></form></td></tr>`).join('') || '<tr><td colspan="4">No school members.</td></tr>';
        ...
        document.querySelector('#invite-branch-form').addEventListener('submit', async event => {
            event.preventDefault();
            const formData = new FormData(event.currentTarget);
            if (!window.confirm('Create this branch invitation with the selected roles? The invitation will be audited and expire in 48 hours.')) return;
            try {
                const invitation = await request(`/superadmin/schools/${schoolId}/branches/${formData.get('branch_id')}/invitations`, {method: 'POST', headers: {'X-CSRF-TOKEN': csrf, 'Content-Type': 'application/json'}, body: JSON.stringify({name: formData.get('name'), email: formData.get('email'), roles: formData.getAll('roles')})});
        ...
        document.querySelectorAll('.branch-access-form').forEach(form => form.addEventListener('submit', async event => {
            ...
            const formData = new FormData(event.currentTarget);
            try {
                await request(`/superadmin/schools/${event.currentTarget.dataset.school}/members/${event.currentTarget.dataset.user}/branch-access`, {method: 'PUT', headers: {'X-CSRF-TOKEN': csrf, 'Content-Type': 'application/json'}, body: JSON.stringify({branch_id: Number(formData.get('branch_id')), roles: formData.getAll('roles'), status: 'active'})});
    ...
    const load = async () => {
        ...
        const primaryMetrics = [['schools', 'Schools'], ['active_schools', 'Active schools'], ['branches', 'Branches'], ['active_branches', 'Active branches'], ['students', 'Students'], ['accounts', 'Accounts'], ['teachers', 'Teachers'], ['open_invoices', 'Open invoices']];
        summary.innerHTML = primaryMetrics.map(([key, label]) => `<article><strong>${esc(data.summary[key])}</strong><span>${label}</span></article>`).join('');
        document.querySelector('#platform-primary-kpis').innerHTML = [['Schools', data.summary.schools, `${data.summary.active_schools} active · ${data.summary.suspended_schools} suspended`], ['Branches', data.summary.branches, `${data.summary.active_branches} active · ${data.summary.suspended_branches} suspended`], ['Students', data.summary.students, `${data.summary.enrollments} enrollments`], ['Accounts', data.summary.accounts, `${data.summary.teachers} teachers · ${data.summary.staff} staff`], ['Projected MRR', `$${mrr.toFixed(2)}`, `${data.summary.open_invoices} open invoices`]].map(([label, value, note]) => `<article><span class="overview-kpi-label">${label}</span><strong>${esc(value)}</strong><small>${esc(note)}</small></article>`).join('');
        const healthBars = [['Active schools', data.summary.active_schools, data.summary.schools, 'active'], ['Suspended schools', data.summary.suspended_schools, data.summary.schools, 'suspended'], ['Active branches', data.summary.active_branches, data.summary.branches, 'active'], ['Suspended branches', data.summary.suspended_branches, data.summary.branches, 'suspended']];
        document.querySelector('#school-health-chart').innerHTML = healthBars.map(([label, value, total, tone]) => `<div class="status-bar-row"><div><span>${label}</span><strong>${esc(value)}</strong></div><div class="status-bar-track"><i class="${tone}" style="width:${total ? Math.max(0, Math.min(100, (Number(value) / Number(total)) * 100)) : 0}%"></i></div></div>`).join('');
        const composition = [['Students', data.summary.students], ['Memberships', data.summary.members], ['Attendance records', data.summary.attendance], ['Payments', data.summary.payments], ['Notifications', data.summary.notifications]];
        const maxComposition = Math.max(...composition.map(([, value]) => Number(value) || 0), 1);
        document.querySelector('#platform-composition-chart').innerHTML = composition.map(([label, value]) => `<div class="metric-bar-row"><div><span>${label}</span><strong>${esc(value)}</strong></div><div class="metric-bar-track"><i style="width:${Math.max(3, (Number(value) / maxComposition) * 100)}%"></i></div></div>`).join('');
        const inventoryMetrics = [['schools', 'Schools'], ['branches', 'Branches'], ['members', 'Memberships'], ['branch_access', 'Branch access'], ['accounts', 'Accounts'], ['students', 'Students'], ['staff', 'Staff'], ['teachers', 'Teachers'], ['classes', 'Classes'], ['subjects', 'Subjects'], ['enrollments', 'Enrollments'], ['attendance', 'Attendance'], ['exams', 'Exams'], ['grades', 'Grades'], ['invoices', 'Invoices'], ['payments', 'Payments'], ['expenses', 'Expenses'], ['payroll', 'Payroll'], ['notices', 'Notices'], ['notifications', 'Notifications'], ['audit', 'Audit events'], ['entitlement_alerts', 'Alerts']];
        document.querySelector('#platform-inventory').innerHTML = inventoryMetrics.map(([key, label]) => `<span><strong>${esc(data.summary[key] ?? 0)}</strong><small>${label}</small></span>`).join('');        schools.innerHTML = data.schools.map(school => { const defaultBranch = school.branch_options.find(branch => branch.is_default) || school.branch_options[0]; const activeBranches = school.active_branches ?? school.branches; const branchLimit = school.max_branches ? `${esc(activeBranches)} active / ${esc(school.max_branches)}<small>${esc(school.branches)} total</small>` : `${esc(activeBranches)} active<small>${esc(school.branches)} total</small>`; const studentLimit = school.max_students ? `${esc(school.students)} / ${esc(school.max_students)}` : `${esc(school.students)} / unlimited`; return `<tr data-school-name="${esc(school.name)}" data-school-slug="${esc(school.slug)}" data-school-status="${esc(school.status)}"><td><button class="platform-school-detail" data-id="${school.id}" type="button"><strong>${esc(school.name)}</strong></button><small>${esc(school.slug)}</small></td><td><span class="platform-status ${esc(school.status)}">${esc(school.status)}</span></td><td>${esc(school.plan_name || 'No plan')}<small>${esc(school.subscription_status || 'unassigned')}</small></td><td>${branchLimit}</td><td>${esc(school.members)}</td><td>${studentLimit}</td><td>${esc(school.staff)}</td><td>${esc(school.teachers)}</td><td>${esc(school.open_invoices)}</td><td>${defaultBranch ? `<button class="platform-workspace" data-school="${esc(school.id)}" data-branch="${esc(defaultBranch.id)}">Open workspace</button>` : ''} <button class="platform-action" data-id="${school.id}" data-status="${school.status === 'active' ? 'suspended' : 'active'}">${school.status === 'active' ? 'Suspend' : 'Activate'}</button></td></tr>`; }).join('') || '<tr><td colspan="10">No schools registered.</td></tr>';
    ...
    const loadUsers = async (page = 1) => {
        ...
        if (userSchoolStatus.value) params.set('school_status', userSchoolStatus.value);
        if (userBranchStatus.value) params.set('branch_status', userBranchStatus.value);
        ...
        userRows.innerHTML = data.users.data.map(user => {
            const access = user.is_superadmin ? 'Platform Superadmin' : (user.access.map(item => `${esc(item.school_name)} / ${esc(item.branch_name)} (${esc(item.roles.join(', '))}) · ${esc(item.school_status === 'active' ? 'School active' : `School ${item.school_status || 'unknown'}`)} · ${esc(item.membership_status === 'active' ? 'Membership active' : `Membership ${item.membership_status || 'unknown'}`)} · ${esc(item.branch_status === 'active' ? 'Branch active' : `Branch ${item.branch_status || 'unknown'}`)} · ${esc(item.status === 'active' ? 'Grant active' : 'Grant suspended')}`).join('<br>') || 'No branch access');
    ...
    const loadBranches = async (page = 1) => {
        ...
        branchRows.innerHTML = data.branches.data.map(branch => `<tr><td>${esc(branch.school_name)}</td><td><strong>${esc(branch.name)}</strong><small>${esc(branch.code)}${branch.is_default ? ' · Default' : ''}</small></td><td><span class="platform-status ${esc(branch.status)}">${esc(branch.status)}</span></td><td>${esc(branch.members)}</td><td>${esc(branch.students)}</td><td>${esc(branch.staff)}</td><td>${esc(branch.teachers)}</td><td>${esc(branch.guardians)}</td><td>${esc(branch.classes)}</td><td>${esc(branch.open_invoices)}</td><td>${esc(branch.academic_years)} years · ${esc(branch.subjects)} subjects · ${esc(branch.attendance)} attendance · ${esc(branch.timetables)} timetable · ${esc(branch.exams)} exams · ${esc(branch.grades)} grades · ${esc(branch.enrollments)} enrollments · ${esc(branch.payments)} payments · ${esc(branch.expenses)} expenses · ${esc(branch.leave_requests)} leave · ${esc(branch.payroll)} payroll · ${esc(branch.payroll_payments)} payroll payments · ${esc(branch.notices)} notices · ${esc(branch.invitations)} invitations · ${esc(branch.notifications)} notifications · ${esc(branch.notification_deliveries)} deliveries</td><td><button class="platform-branch-workspace" data-school="${esc(branch.school_id)}" data-branch="${esc(branch.id)}" type="button">Open workspace</button> <button class="platform-branch-status" data-school="${esc(branch.school_id)}" data-branch="${esc(branch.id)}" data-status="${branch.status === 'active' ? 'suspended' : 'active'}">${branch.status === 'active' ? 'Suspend' : 'Activate'}</button></td></tr>`).join('') || '<tr><td colspan="12">No matching branches.</td></tr>';
        ...
        branchPagination.querySelectorAll('[data-branch-page]').forEach(button => button.addEventListener('click', () => loadBranches(Number(button.dataset.branchPage)).catch(error => { branchRows.innerHTML = `<tr><td colspan="12">${esc(error.message)}</td></tr>`; })));
    ...
    document.querySelector('#branch-search-form').addEventListener('submit', event => { event.preventDefault(); loadBranches().catch(error => { branchRows.innerHTML = `<tr><td colspan="12">${esc(error.message)}</td></tr>`; }); });
    branchSchool.addEventListener('change', () => loadBranches().catch(error => { branchRows.innerHTML = `<tr><td colspan="12">${esc(error.message)}</td></tr>`; }));
    ...
    inventoryToggle.addEventListener('click', () => {
        const inventory = document.querySelector('#platform-inventory');
        inventory.hidden = !inventory.hidden;
        inventoryToggle.textContent = inventory.hidden ? 'Show all metrics' : 'Hide metrics';
    });
    document.querySelector('#overview-refresh').addEventListener('click', event => {
        event.currentTarget.disabled = true;
        load().catch(error => window.alert(error.message)).finally(() => { event.currentTarget.disabled = false; });
    ...
    loadBranches().catch(error => { branchRows.innerHTML = `<tr><td colspan="12">${esc(error.message)}</td></tr>`; });
})();
</script>
</body>
</html>
