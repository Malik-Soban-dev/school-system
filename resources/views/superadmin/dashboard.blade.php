...
(() => {
    ...
    const loadUsers = async (page = 1) => {
        ...
        userRows.innerHTML = data.users.data.map(user => {
            const access = user.is_superadmin ? 'Platform Superadmin' : (user.access.map(item => `${esc(item.school_name)} / ${esc(item.branch_name)} (${esc(item.roles.join(', '))}) · ${esc(item.membership_status === 'active' ? 'School active' : `School ${item.membership_status || 'unknown'}`)} · ${esc(item.status === 'active' ? 'Branch active' : 'Branch suspended')}`).join('<br>') || 'No branch access');
