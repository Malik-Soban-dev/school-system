...
(() => {
    ...
    const load = async () => {
        ...
        summary.innerHTML = [['schools','Schools'],['active_schools','Active schools'],['suspended_schools','Suspended schools'],['branches','Branches'],['active_branches','Active branches'],['suspended_branches','Suspended branches'],['members','Memberships'],['branch_access','Branch access grants'],['accounts','Accounts'],['students','Students'],['staff','Staff'],['teachers','Teachers'],['classes','Classes'],['academic_years','Academic years'],['subjects','Subjects'],['teacher_assignments','Teaching assignments'],['guardians','Guardians'],['guardian_links','Guardian links'],['enrollments','Enrollments'],['attendance','Attendance'],['timetables','Timetables'],['exams','Exams'],['exam_subjects','Exam subjects'],['grade_bands','Grading bands'],['grades','Grades'],['invoices','Invoices'],['payments','Payments'],['expenses','Expenses'],['leave_requests','Leave requests'],['payroll','Payroll'],['payroll_payments','Payroll payments'],['notices','Notices'],['invitations','Invitations'],['notifications','Notifications'],['notification_events','Notification events'],['notification_deliveries','Notification deliveries'],['notification_preferences','Notification preferences'],['settings','Settings'],['audit','Audit events'],['open_invoices','Open invoices'],['mrr','Projected MRR'],['past_due','Past due'],['entitlement_alerts','Alerts']].map(([key,label]) => { const value = key === 'mrr' ? `$${mrr.toFixed(2)}` : key === 'past_due' ? (data.billing?.subscriptions?.past_due || 0) : data.summary[key]; return `<article><strong>${esc(value)}</strong><span>${label}</span></article>`; }).join('');
        schools.innerHTML = data.schools.map(school => { const defaultBranch = school.branch_options.find(branch => branch.is_default) || school.branch_options[0]; const activeBranches = school.active_branches ?? school.branches; const branchLimit = school.max_branches ? `${esc(activeBranches)} active / ${esc(school.max_branches)}<small>${esc(school.branches)} total</small>` : `${esc(activeBranches)} active<small>${esc(school.branches)} total</small>`; const studentLimit = school.max_students ? `${esc(school.students)} / ${esc(school.max_students)}` : `${esc(school.students)} / unlimited`; return `<tr data-school-name="${esc(school.name)}" data-school-slug="${esc(school.slug)}" data-school-status="${esc(school.status)}"><td><button class="platform-school-detail" data-id="${school.id}" type="button"><strong>${esc(school.name)}</strong></button><small>${esc(school.slug)}</small></td><td><span class="platform-status ${esc(school.status)}">${esc(school.status)}</span></td><td>${esc(school.plan_name || 'No plan')}<small>${esc(school.subscription_status || 'unassigned')}</small></td><td>${branchLimit}</td><td>${esc(school.members)}</td><td>${studentLimit}</td><td>${esc(school.staff)}</td><td>${esc(school.teachers)}</td><td>${esc(school.open_invoices)}</td><td>${defaultBranch ? `<button class="platform-workspace" data-school="${esc(school.id)}" data-branch="${esc(defaultBranch.id)}">Open workspace</button>` : ''} <button class="platform-action" data-id="${school.id}" data-status="${school.status === 'active' ? 'suspended' : 'active'}">${school.status === 'active' ? 'Suspend' : 'Activate'}</button></td></tr>`; }).join('') || '<tr><td colspan="10">No schools registered.</td></tr>';
