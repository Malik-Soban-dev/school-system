...
(() => {
    ...
    const showSchoolDetail = async schoolId => {
        ...
        existingUserPanel.querySelector('#existing-user-access-form').addEventListener('submit', async event => {
            ...
            try {
                await request(`/superadmin/schools/${schoolId}/members/${formData.get('user_id')}/branch-access`, {method: 'PUT', headers: {'X-CSRF-TOKEN': csrf, 'Content-Type': 'application/json'}, body: JSON.stringify({branch_id: Number(formData.get('branch_id')), roles: [formData.get('role')], status: 'active'})});
