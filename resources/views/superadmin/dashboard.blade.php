...
(() => {
    ...
    const showSchoolDetail = async schoolId => {
        ...
        const branchRows = data.branches.map(branch => `<tr><td>${esc(branch.name)}${branch.is_default ? ' <small>Default</small>' : ''}</td><td>${esc(branch.code)}</td><td><span class="platform-status ${esc(branch.status)}">${esc(branch.status)}</span></td><td>${esc(branch.members)} members · ${esc(branch.students)} students · ${esc(branch.staff)} staff · ${esc(branch.teachers)} teachers · ${esc(branch.classes)} classes · ${esc(branch.open_invoices)} open invoices</td><td><button class="branch-workspace" data-branch="${esc(branch.id)}">Open workspace</button> <button class="branch-edit" data-branch="${esc(branch.id)}" data-name="${esc(branch.name)}" data-code="${esc(branch.code)}">Edit</button> ${branch.is_default ? '<small>Default</small>' : `<button class="branch-default" data-branch="${esc(branch.id)}">Set default</button>`} <button class="branch-status" data-branch="${esc(branch.id)}" data-status="${branch.status === 'active' ? 'suspended' : 'active'}">${branch.status === 'active' ? 'Suspend' : 'Activate'}</button></td></tr>`).join('') || '<tr><td colspan="5">No branches registered.</td></tr>';
    ...
    const load = async () => {
        ...
        summary.innerHTML = [['schools','Schools'],['branches','Branches'],['members','Memberships'],['accounts','Accounts'],['students','Students'],['staff','Staff'],['teachers','Teachers'],['classes','Classes'],['guardians','Guardians'],['enrollments','Enrollments'],['attendance','Attendance'],['exams','Exams'],['grades','Grades'],['invoices','Invoices'],['payments','Payments'],['expenses','Expenses'],['leave_requests','Leave requests'],['payroll','Payroll'],['payroll_payments','Payroll payments'],['notices','Notices'],['notifications','Notifications'],['notification_deliveries','Notification deliveries'],['open_invoices','Open invoices'],['mrr','Projected MRR'],['past_due','Past due'],['entitlement_alerts','Alerts']].map(([key,label]) => { const value = key === 'mrr' ? `$${mrr.toFixed(2)}` : key === 'past_due' ? (data.billing?.subscriptions?.past_due || 0) : data.summary[key]; return `<article><strong>${esc(value)}</strong><span>${label}</span></article>`; }).join('');
    ...
    const loadUsers = async (page = 1) => {
        ...
        userRows.innerHTML = data.users.data.map(user => {
            const access = user.is_superadmin ? 'Platform Superadmin' : (user.access.map(item => `${esc(item.school_name)} / ${esc(item.branch_name)} (${esc(item.roles.join(', '))}) · ${esc(item.membership_status === 'active' ? 'School active' : `School ${item.membership_status || 'unknown'}`)} · ${esc(item.branch_status === 'active' ? 'Branch active' : `Branch ${item.branch_status || 'unknown'}`)} · ${esc(item.status === 'active' ? 'Grant active' : 'Grant suspended')}`).join('<br>') || 'No branch access');
