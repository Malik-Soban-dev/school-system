...
<div class="split">
    ...
    <section class="form-panel" aria-labelledby="mfa-title"><div class="eyebrow">Authenticator verification</div><h2 id="mfa-title">Verify your identity</h2><form action="{{ route('mfa.challenge.verify') }}" method="POST">@csrf<div class="field"><label for="code">Authenticator code</label><input id="code" name="code" inputmode="numeric" pattern="[0-9 ]{6,7}" autocomplete="one-time-code" required autofocus></div><button class="button full" type="submit">Continue to Superadmin</button></form><p class="helper" style="margin-top:24px">If you cannot access the authenticator, use the controlled password-reset process from your platform operator.</p></section>
</div>
@endsection
