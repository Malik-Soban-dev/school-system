...
<div class="workspace">
    ...
    <div class="content">
        <header><button class="mobile-toggle" aria-expanded={menu} onclick={() => menu = !menu}>{$translator("☰ Menu")}</button>{#if meta.contexts?.length > 1}<label class="context-switcher"><span>{$translator("Workspace")}</span><select value={`${meta.current_context.school_id}:${meta.current_context.branch_id}`} onchange={switchContext} disabled={busy}>{#each meta.contexts as context}<option value={`${context.school_id}:${context.branch_id}`}>{context.school_name} · {context.branch_name}</option>{/each}</select></label>{/if}<span class="role">{meta.user.roles.map(role => $translator(role)).join(' · ')}</span><Preferences /><div class="account"><a href="/account">{$translator("Account")}</a><form action="/logout" method="post"><input type="hidden" name="_token" value={csrf}><button>{$translator("Sign out")}</button></form></div></header>
        <main id="main">
            ...
            {:else if definition}
                ...
                <div class="records">{#each rows as row}<article class="record" data-cy="record"><h2>{row.name ?? row.title ?? row.reference ?? `${definition.singular} #${row.id}`}</h2><dl>{#each definition.fields as field}<div><dt>{$translator(field.label)}</dt><dd>{display(field, row[field.name])}</dd></div>{/each}{#if row.payment_status}<div><dt>{$translator("Payment status")}</dt><dd>{row.payment_status}</dd></div><div><dt>{$translator("Amount paid")}</dt><dd>{display({type: 'money'}, row.paid)}</dd></div>{/if}{#if row.balance !== undefined}<div><dt>{$translator("Outstanding balance")}</dt><dd>{display({type: 'money'}, row.balance)}</dd></div>{/if}{#if row.net !== undefined}<div><dt>{$translator("Net pay")}</dt><dd>{display({type: 'money'}, row.net)}</dd></div>{/if}{#if row.percentage !== undefined}<div><dt>{$translator("Percentage")}</dt><dd>{row.percentage}%</dd></div>{/if}</dl><div class="actions">{#if section === 'grades' && meta.user.roles.some(role => ['owner','admin','parent','student'].includes(role))}<a href={`/report-cards/${row.exam_id}/${row.student_id}`} target="_blank" rel="noopener">{$translator("Full report card")}</a>{/if}{#if definition.canWrite && !definition.immutable}<Button color="alternative" onclick={() => openEditor(row)}>{$translator("Edit")}</Button>{/if}{#if ['payments','invoices','payroll','payroll_payments','grades'].includes(section)}<a href={`/reports/${section}/${row.id}`} target="_blank" rel="noopener">{$translator("Print / save PDF")}</a>{/if}</div></article>{/each}</div>
