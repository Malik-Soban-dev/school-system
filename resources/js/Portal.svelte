...
<div class="workspace">
    ...
    <div class="content">
        <header><button class="mobile-toggle" aria-expanded={menu} onclick={() => menu = !menu}>{$translator("☰ Menu")}</button><span class="role">{meta.user.roles.map(role => $translator(role)).join(' · ')}</span><Preferences /><div class="account"><a href="/account">{$translator("Account")}</a><form action="/logout" method="post"><input type="hidden" name="_token" value={csrf}><button>{$translator("Sign out")}</button></form></div></header>
        <main id="main">
