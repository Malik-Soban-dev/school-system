<script>
    ...
    let steps = $derived([
        {title: `Welcome to ${section === 'overview' ? 'your workspace' : title}`, body: definition?.help ?? 'Use the menu to open your school tools. You only see information your role is allowed to access.'},
    ...
    async function finishGuide() {
        guide = false;
        try { await api('/portal/tutorial', 'PUT', {module: section}); meta.user.tutorials = [...(meta.user.tutorials ?? []), section]; } catch(e) { error = e.message; }
    }
    ...
    function display(field, value) {
        if (value === null || value === '') return 'ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â';
...
<svelte:head><title>{title} ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· School workspace</title></svelte:head>
...
<div class="workspace">
    ...
    <div class="content">
        <header><button class="mobile-toggle" aria-expanded={menu} onclick={() => menu = !menu}>ÃƒÆ’Ã‚Â¢Ãƒâ€¹Ã…â€œÃƒâ€šÃ‚Â° Menu</button><span class="role">{meta.user.roles.join(' ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· ')}</span><div class="account"><a href="/account">Account</a><form action="/logout" method="post"><input type="hidden" name="_token" value={csrf}><button>Sign out</button></form></div></header>
        <main id="main">
            <div class="heading"><div><p class="eyebrow">YOUR SCHOOL WORKSPACE</p><h1>{title}</h1><p>{definition?.help ?? `Welcome, ${meta.user.name}. Let's make today a good school day.`}</p></div>{#if section !== 'audit'}<Button color="alternative" onclick={() => {step = 0; guide = true;}}>Page guide</Button>{/if}</div>
            ...
            {#if loading}<div class="empty"><Spinner /> <p>Loading your workspaceÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦</p></div>
            {:else if section === 'overview'}
                ...
                <div class="tiles">{#each meta.modules as item}<button class="tile" onclick={() => navigate(item.key)}><span class="tile-icon">{item.label.slice(0, 1)}</span><h2>{item.label}</h2><p>{item.help}</p><span class="open">Open tool ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢</span></button>{/each}</div>
            {:else if definition}
                <div class="toolbar"><form onsubmit={(e) => {e.preventDefault(); page = 1; loadRows();}}><label class="sr-only" for="search">Search records</label><input id="search" bind:value={search} placeholder="Search name or referenceÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦" maxlength="100"><Button type="submit" color="alternative">Search</Button></form>{#if definition.canWrite}<Button onclick={() => openEditor()} data-cy="add-record">Add {definition.singular}</Button>{/if}</div>
            ...
            {:else if section === 'people'}
                ...
                <div class="records people">{#each rows as person}<article class="record"><h2>{person.name}</h2><p>{person.username || person.email}</p><p>{person.roles?.join(' ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· ')} ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· {person.is_active ? 'Active' : 'Suspended'}</p>{#if person.id !== meta.user.id && !person.roles?.includes('owner') && (meta.user.roles.includes('owner') || !person.roles?.includes('admin'))}<Button color="alternative" onclick={() => {personToEdit = person; selectedRoles = [...person.roles]; errors = {}; access = true;}}>Edit roles</Button> <Button color="alternative" disabled={busy} onclick={() => updateAccess(person)}>{person.is_active ? 'Suspend access' : 'Restore access'}</Button>{/if}</article>{/each}</div>
            ...
            {:else if section === 'audit'}<div class="records">{#each rows as row}<article class="record"><h2>{row.action.replaceAll('_',' ')}</h2><p>{row.name ?? 'Former account'} ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· {row.module.replaceAll('_',' ')} #{row.record_id}</p><p>{row.created_at}</p></article>{/each}</div>{/if}
        ...
        </main><footer>School System ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· Built around your school day</footer>
    </div>
</div>
