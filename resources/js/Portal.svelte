    ...
    <div class="content">
        ...
        <main id="main">
            ...
            {:else if section === 'people'}
                ...
                <div class="records people">{#each rows as person}<article class="record"><h2>{person.name}</h2><p>{person.username || person.email}</p><p>{person.roles?.join(' · ')} · {$translator(person.is_active ? 'Active' : 'Suspended')}</p>{#if person.id !== meta.user.id && !person.roles?.includes('owner') && (meta.user.roles.includes('owner') || !person.roles?.includes('admin'))}<Button color="alternative" onclick={() => {personToEdit = person; selectedRoles = [...person.roles]; errors = {}; access = true;}}>{$translator("Edit roles")}</Button> <Button color="alternative" disabled={busy} onclick={() => updateAccess(person)}>{$translator(person.is_active ? 'Suspend access' : 'Restore access')}</Button>{/if}</article>{/each}</div>
