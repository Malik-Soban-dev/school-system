...
class PlatformController extends Controller
{
    ...
    public function records(Request $request, int $school, string $module): JsonResponse
    {
        ...
        $query = match ($module) {
            ...
            })->where('r.school_id', $school)->when($branchId !== null, fn ($q) => $q->where('r.branch_id', $branchId))->when($search !== '', fn ($q) => $q->where(fn ($s) => $s->where('u.name', 'like', '%'.$search.'%')->orWhere('u.email', 'like', '%'.$search.'%')->orWhere('b.name', 'like', '%'.$search.'%')->orWhere('b.code', 'like', '%'.$search.'%')))->orderBy('u.name')->orderBy('b.name')->select(['r.id', 'u.name', 'u.email', 'u.username', 'b.name as branch_name', 'b.code as branch_code', 'r.roles', 'r.status', 'membership.status as membership_status', 'r.created_at', 'r.updated_at']),
