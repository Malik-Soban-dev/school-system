...
class PlatformController extends Controller
{
    ...
    public function records(Request $request, int $school, string $module): JsonResponse
    {
        ...
        $query = match ($module) {
            ...
            'teacher_assignments' => DB::table('school_teacher_assignments as r')->join('users as u', 'u.id', '=', 'r.user_id')->join('school_classes as c', 'c.id', '=', 'r.class_id')->join('school_subjects as s', 's.id', '=', 'r.subject_id')->where('r.school_id', $school)->when($branchId !== null, fn ($q) => $q->where('r.branch_id', $branchId))->when($search !== '', fn ($q) => $q->where(fn ($x) => $x->where('u.name', 'like', '%'.$search.'%')->orWhere('c.name', 'like', '%'.$search.'%')->orWhere('s.name', 'like', '%'.$search.'%')))->orderBy('u.name')->select(['r.id', 'u.name as teacher', 'c.name as class_name', 's.name as subject_name', 'r.status', 'r.created_at']),
            'guardian_links' => DB::table('school_guardian_links as r')->join('school_students as s', 's.id', '=', 'r.student_id')->join('users as u', 'u.id', '=', 'r.user_id')->where('r.school_id', $school)->when($branchId !== null, fn ($q) => $q->where('r.branch_id', $branchId))->when($search !== '', fn ($q) => $q->where(fn ($x) => $x->where('s.name', 'like', '%'.$search.'%')->orWhere('u.name', 'like', '%'.$search.'%')))->orderBy('s.name')->select(['r.id', 's.name as student', 'u.name as guardian', 'r.relationship', 'r.status', 'r.created_at']),
            'attendance' => DB::table('school_attendance as r')->join('school_students as s', 's.id', '=', 'r.student_id')->where('r.school_id', $school)->when($branchId !== null, fn ($q) => $q->where('r.branch_id', $branchId))->when($search !== '', fn ($q) => $q->where(fn ($x) => $x->where('s.name', 'like', '%'.$search.'%')->orWhere('r.status', 'like', '%'.$search.'%')))->orderByDesc('r.date')->select(['r.id', 's.name as student', 'r.date', 'r.status', 'r.note', 'r.created_at']),
            'timetables' => DB::table('school_timetables as r')->join('school_classes as c', 'c.id', '=', 'r.class_id')->join('school_subjects as s', 's.id', '=', 'r.subject_id')->join('users as u', 'u.id', '=', 'r.teacher_id')->where('r.school_id', $school)->when($branchId !== null, fn ($q) => $q->where('r.branch_id', $branchId))->when($search !== '', fn ($q) => $q->where(fn ($x) => $x->where('c.name', 'like', '%'.$search.'%')->orWhere('s.name', 'like', '%'.$search.'%')->orWhere('u.name', 'like', '%'.$search.'%')))->orderBy('r.weekday')->select(['r.id', 'r.weekday', 'r.starts_at', 'r.ends_at', 'r.room', 'c.name as class_name', 's.name as subject_name', 'u.name as teacher']),
            'exams' => DB::table('school_exams as r')->join('school_classes as c', 'c.id', '=', 'r.class_id')->where('r.school_id', $school)->when($branchId !== null, fn ($q) => $q->where('r.branch_id', $branchId))->when($search !== '', fn ($q) => $q->where(fn ($x) => $x->where('r.name', 'like', '%'.$search.'%')->orWhere('c.name', 'like', '%'.$search.'%')))->orderByDesc('r.date')->select(['r.id', 'r.name', 'c.name as class_name', 'r.date', 'r.status', 'r.schedule_status', 'r.created_at']),
            ...
            'exam_subjects' => DB::table('school_exam_subjects as r')->join('school_exams as e', 'e.id', '=', 'r.exam_id')->join('school_subjects as s', 's.id', '=', 'r.subject_id')->where('r.school_id', $school)->when($branchId !== null, fn ($q) => $q->where('r.branch_id', $branchId))->when($search !== '', fn ($q) => $q->where(fn ($x) => $x->where('e.name', 'like', '%'.$search.'%')->orWhere('s.name', 'like', '%'.$search.'%')))->orderByDesc('r.id')->select(['r.id', 'e.name as exam', 's.name as subject', 'r.maximum', 'r.weight', 'r.created_at']),
            'grades' => DB::table('school_grades as r')->join('school_exams as e', 'e.id', '=', 'r.exam_id')->join('school_students as s', 's.id', '=', 'r.student_id')->join('school_subjects as u', 'u.id', '=', 'r.subject_id')->where('r.school_id', $school)->when($branchId !== null, fn ($q) => $q->where('r.branch_id', $branchId))->when($search !== '', fn ($q) => $q->where(fn ($x) => $x->where('s.name', 'like', '%'.$search.'%')->orWhere('e.name', 'like', '%'.$search.'%')->orWhere('u.name', 'like', '%'.$search.'%')))->orderByDesc('r.id')->select(['r.id', 'e.name as exam', 's.name as student', 'u.name as subject', 'r.marks', 'r.maximum', 'r.remarks', 'r.created_at']),
            ...
            'payroll' => DB::table('school_payroll as r')->join('school_staff as s', 's.id', '=', 'r.staff_id')->where('r.school_id', $school)->when($branchId !== null, fn ($q) => $q->where('r.branch_id', $branchId))->when($search !== '', fn ($q) => $q->where(fn ($x) => $x->where('s.name', 'like', '%'.$search.'%')->orWhere('r.month', 'like', '%'.$search.'%')))->orderByDesc('r.month')->select(['r.id', 's.name as staff_member', 'r.month', 'r.basic', 'r.allowances', 'r.deductions', 'r.created_at']),
            'payroll_payments' => DB::table('school_payroll_payments as r')->join('school_payroll as p', 'p.id', '=', 'r.payroll_id')->join('school_staff as s', 's.id', '=', 'p.staff_id')->where('r.school_id', $school)->when($branchId !== null, fn ($q) => $q->where('r.branch_id', $branchId))->when($search !== '', fn ($q) => $q->where(fn ($x) => $x->where('r.reference', 'like', '%'.$search.'%')->orWhere('s.name', 'like', '%'.$search.'%')))->orderByDesc('r.paid_on')->select(['r.id', 'r.reference', 's.name as staff_member', 'r.amount', 'r.paid_on', 'r.method', 'r.created_at']),
            ...
            'enrollments' => DB::table('school_enrollments as r')->join('school_students as s', 's.id', '=', 'r.student_id')->join('school_classes as c', 'c.id', '=', 'r.class_id')->where('r.school_id', $school)->when($branchId !== null, fn ($q) => $q->where('r.branch_id', $branchId))->when($search !== '', fn ($q) => $q->where(fn ($x) => $x->where('s.name', 'like', '%'.$search.'%')->orWhere('c.name', 'like', '%'.$search.'%')))->orderByDesc('r.id')->select(['r.id', 's.name as student', 'c.name as class_name', 'r.created_at']),
            ...
            'notification_deliveries' => DB::table('school_notification_deliveries as r')->join('school_notifications as n', 'n.id', '=', 'r.notification_id')->join('users as u', 'u.id', '=', 'n.user_id')->where('r.school_id', $school)->when($branchId !== null, fn ($q) => $q->where('r.branch_id', $branchId))->when($search !== '', fn ($q) => $q->where(fn ($x) => $x->where('r.channel', 'like', '%'.$search.'%')->orWhere('r.status', 'like', '%'.$search.'%')->orWhere('r.error_code', 'like', '%'.$search.'%')->orWhere('u.name', 'like', '%'.$search.'%')))->orderByDesc('r.id')->select(['r.id', 'u.name as recipient', 'n.title', 'r.channel', 'r.status', 'r.attempts', 'r.error_code', 'r.provider_id', 'r.available_at', 'r.updated_at']),
            'settings' => DB::table('school_settings as r')->where('r.school_id', $school)->when($search !== '', fn ($q) => $q->where(fn ($x) => $x->where('r.key', 'like', '%'.$search.'%')->orWhere('r.value', 'like', '%'.$search.'%')))->orderBy('r.key')->select(['r.school_id', 'r.key'])->selectRaw("case when r.key = 'logo_data' then '[redacted asset]' else r.value end as value"),
