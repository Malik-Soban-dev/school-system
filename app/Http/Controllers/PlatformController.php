...
class PlatformController extends Controller
{
    ...
    public function records(Request $request, int $school, string $module): JsonResponse
    {
        ...
        $query = match ($module) {
            ...
            'fee_concessions' => DB::table('school_fee_concessions as r')->join('school_students as s', function ($join) use ($branchId): void {
                $join->on('s.id', '=', 'r.student_id')->when($branchId !== null, fn ($q) => $q->where('s.branch_id', $branchId));
            })->where('r.school_id', $school)->when($branchId !== null, fn ($q) => $q->where('r.branch_id', $branchId))->when($search !== '', fn ($q) => $q->where(fn ($x) => $x->where('s.name', 'like', '%'.$search.'%')->orWhere('r.name', 'like', '%'.$search.'%')))->orderByDesc('r.id')->select(['r.id', 's.name as student_name', 'r.name', 'r.type', 'r.value', 'r.starts_on', 'r.ends_on', 'r.status', 'r.created_at']),
            'fee_plans' => DB::table('school_fee_plans as r')->join('school_classes as c', function ($join) use ($branchId): void {
                $join->on('c.id', '=', 'r.class_id')->when($branchId !== null, fn ($q) => $q->where('c.branch_id', $branchId));
            })->where('r.school_id', $school)->when($branchId !== null, fn ($q) => $q->where('r.branch_id', $branchId))->when($search !== '', fn ($q) => $q->where(fn ($x) => $x->where('c.name', 'like', '%'.$search.'%')->orWhere('r.name', 'like', '%'.$search.'%')))->orderBy('c.name')->select(['r.id', 'c.name as class_name', 'r.name', 'r.amount', 'r.due_day', 'r.status', 'r.created_at']),
