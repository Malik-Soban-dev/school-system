...
class PlatformController extends Controller
{
    ...
    public function users(Request $request): JsonResponse
    {
        ...
        })->whereIn('a.user_id', $userIds)->get(['a.user_id', 's.id as school_id', 's.name as school_name', 'b.id as branch_id', 'b.name as branch_name', 'b.status as branch_status', 'a.roles', 'a.status', 'membership.status as membership_status'])->groupBy('user_id');

        return response()->json(['users' => $users->through(function (object $user) use ($access): array {
            $roles = json_decode((string) $user->roles, true) ?: [];

            return ['id' => $user->id, 'name' => $user->name, 'username' => $user->username, 'email' => $user->email, 'roles' => $roles, 'is_active' => (bool) $user->is_active, 'is_superadmin' => in_array('superadmin', $roles, true), 'access' => $access->get($user->id, collect())->map(fn (object $row): array => ['school_id' => $row->school_id, 'school_name' => $row->school_name, 'branch_id' => $row->branch_id, 'branch_name' => $row->branch_name, 'branch_status' => $row->branch_status, 'roles' => json_decode((string) $row->roles, true) ?: [], 'status' => $row->status, 'membership_status' => $row->membership_status])->values()->all()];
        })]);
    }
    ...
    public function records(Request $request, int $school, string $module): JsonResponse
    {
        ...
        $query = match ($module) {
            ...
            'branch_access' => DB::table('school_user_branches as r')->join('users as u', 'u.id', '=', 'r.user_id')->join('school_branches as b', 'b.id', '=', 'r.branch_id')->leftJoin('school_user as membership', function ($join): void {
                $join->on('membership.user_id', '=', 'r.user_id')->on('membership.school_id', '=', 'r.school_id');
            })->where('r.school_id', $school)->when($branchId !== null, fn ($q) => $q->where('r.branch_id', $branchId))->when($search !== '', fn ($q) => $q->where(fn ($s) => $s->where('u.name', 'like', '%'.$search.'%')->orWhere('u.email', 'like', '%'.$search.'%')->orWhere('b.name', 'like', '%'.$search.'%')->orWhere('b.code', 'like', '%'.$search.'%')))->orderBy('u.name')->orderBy('b.name')->select(['r.id', 'u.name', 'u.email', 'u.username', 'b.name as branch_name', 'b.code as branch_code', 'b.status as branch_status', 'r.roles', 'r.status', 'membership.status as membership_status', 'r.created_at', 'r.updated_at']),
