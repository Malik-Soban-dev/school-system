...
class BuildPlatformUserExport implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        fputcsv($handle, ['user_id', 'name', 'username', 'email', 'account_status', 'global_roles', 'school', 'school_membership_status', 'branch', 'branch_roles', 'branch_access_status']);
        $rows = 0;
        DB::table('users as u')->select(['u.id', 'u.name', 'u.username', 'u.email', 'u.is_active', 'u.roles'])->orderBy('u.id')->chunkById(500, function ($users) use ($handle, &$rows): void {
            ...
            })->whereIn('a.user_id', $userIds)->orderBy('s.name')->orderBy('b.name')->get(['a.user_id', 's.name as school_name', 'membership.status as school_membership_status', 'b.name as branch_name', 'a.roles', 'a.status'])->groupBy('user_id');
            foreach ($users as $user) {
                ...
                foreach ($grants as $grant) {
                    fputcsv($handle, [$user->id, $user->name, $user->username, $user->email, $user->is_active ? 'active' : 'suspended', implode('|', json_decode((string) $user->roles, true) ?: []), $grant->school_name, $grant->school_membership_status, $grant->branch_name, implode('|', json_decode((string) $grant->roles, true) ?: []), $grant->status]);
                    $rows++;
                }
            }
        }, 'u.id', 'id');
